...
# Когда снаряд начинает дымить. Пуля — сразу, с первого кадра: порох горит в момент
# выстрела, а не когда-то потом. Ракета сходит с крыла холодной и метод переопределяет,
# см. rocket.gd. Стреляющий обязан поставить снаряд на место до добавления в дерево:
# облако зажигания вылетает уже здесь, и из непереставленного снаряда легло бы
# в начало координат.
