...
func _ready() -> void:
	# flip_h переворачивает только картинку, дочерние узлы остаются на месте. Без зеркала
	# ракеты с правого крыла вылетали бы из точки левого, то есть из-под чужой шахты.
	if flip_h:
...
## Открывает или закрывает шахту. Открытая видна на крыле и заряжена, закрытая спрятана
## и в очереди пуска не участвует: так корабль без прокачки несёт меньше ракет, чем
## нарисовано шахт. Открытие — это и есть загрузка: прокачка, добавившая пару шахт,
## сразу даёт по ракете в каждую новую, не трогая уже отстрелянные.
func set_open(open: bool) -> void:
	...
	visible = open
	if open:
		# Шахта могла остаться на последнем кадре пуска — с рассеявшимся дымом вместо
		# ракеты. Перемотка возвращает в неё нарисованную ракету.
		stop()
		frame = 0
