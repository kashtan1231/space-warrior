...
## сигнал столкновения, см. bullet.gd. from — глобальная точка, откуда пришло попадание;
## от неё берётся только сторона, а место на обшивке считается по самой форме.
func ignite(shape: CollisionShape2D, from: Vector2) -> void:
...
func _surface_point(shape: CollisionShape2D, from: Vector2) -> Vector2:
	var to_world := shape.global_transform
	return to_world * _closest_surface_point(shape.shape, to_world.affine_inverse() * from)
...
func _closest_surface_point(shape: Shape2D, point: Vector2) -> Vector2:
